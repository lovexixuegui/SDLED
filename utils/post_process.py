...
def isEdgePoints(img,x1,y1,x2,y2,edge_limit=3):
    x_limit=img.shape[0]-edge_limit
    y_limit=img.shape[1]-edge_limit

    if (x1<edge_limit or x1>=x_limit or y1<edge_limit or y1>=y_limit) and (x2<edge_limit or x2>=x_limit or y2<edge_limit or y2>=y_limit):
        # print("True")
        return True
    else:
        return False
...
def deleteSingleWay(img, singleWay, symbol=255, singleLength=20, final=False):
    x_limit = img.shape[0]
    y_limit = img.shape[1]

    while len(singleWay) > 0:
        xx, yy = singleWay.pop()
        wayLength = 1
        # tempPoints = []
        queuePoints = []
        deletePoints = []
        queuePoints.append((xx, yy))
        while len(queuePoints) > 0:
            x, y = queuePoints.pop()
            neighbor = 0
            tempPoints = []
            # 左边
            ...
            if neighbor <2:
                img[x][y] = 0
                wayLength += 1
                while(len(tempPoints)>0):
                    q_x,q_y=tempPoints.pop()
                    queuePoints.append((q_x,q_y))
                if final == False:
                    deletePoints.append((x, y))
            elif(neighbor==2):
                x1,y1=tempPoints.pop()
                x2,y2=tempPoints.pop()
                if ((x1-x2==0 and abs(y1-y2)==1) or (abs(x1-x2)==1 and y1-y2==0)):
                    img[x][y] = 0
                    wayLength += 1
                    deletePoints.append((x, y))
            else:
